...
public class ItemFragment extends Item implements IFirstTick {
	...
	public static final String[] orenames = new String[] { "Ferrous", "Rare Metal", "Platinum Metal" };
	public static final double[][][] orecontains = new double[][][] { { { 90, 25 }, { 5, 26 }, { 5, 27 } },
	...
	public void getSubItems(Item itemIn, CreativeTabs tab, List subItems) {
		for (int i = 0; i < ItemFragment.names.length * 4 + orenames.length; i++)
	...
	public void addInformation(ItemStack stack, EntityPlayer playerIn, List info, boolean advanced) {
		...
		if (stack.hasTagCompound()) {
			if (!stack.getTagCompound().getBoolean("ore")) {
				...
				double[][] odds = orecontains[stack.getTagCompound().getInteger("id")];
				info.add("Contains: ");
				for (int i = 0; i < odds.length; i++) {
					info.add(((int) odds[i][0]) + "%" + " " + names[(int) odds[i][1]]);
				}
			}
		}
	}
	...
	public void onFirstTick(ItemStack stack) {
		...
		if (d < (ItemFragment.names.length * 4)) {
			...
			stack.getTagCompound().setString("name", orenames[d]);
